...
async function requestClipboardReadPermission(): Promise<boolean> {
  try {
    ...
    if (result.state === 'granted' || result.state === 'prompt') {
      if (result.state === 'prompt' && permissions.request) {
        const requested = await permissions.request({ name: 'clipboard-read' })
...
async function readClipboardText(): Promise<string> {
  if (navigator.clipboard?.readText) {
    try {
      // On Android/Mobile Chrome the Clipboard API is gated behind a permission.
      // A user gesture (button click) is required; requesting here makes it work
      // when the user intentionally taps the paste button.
      await requestClipboardReadPermission()
