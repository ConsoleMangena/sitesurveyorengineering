...
/**
 * Format a survey point reference using Zimbabwe / SURPAC convention:
 * Y = Easting, X = Northing, H = Height/RL.
 */
export function fmtPointRef(
  ...
  return `${p.pointNo} (Y ${fmtCoord(p.e, decimals)}, X ${fmtCoord(p.n, decimals)}${h})`;
}
