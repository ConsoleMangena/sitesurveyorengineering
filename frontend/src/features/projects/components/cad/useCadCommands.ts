...
import { fmtBearing, fmtCoord, fmtDistance, fmtPointRef, parseBearing, type BearingFormat } from "./survey/format.ts";
...
export function runCommand(raw: string, ctx: CommandContext): void {
  const { cad, bearingFormat, setTool, log, fitExtents, layout } = ctx;
  const text = raw.trim();
  ...
  if (upper === "LIST" || upper === "LI") {
    ...
    if (sel.type === "point" && sel.id) {
      ...
      if (p) {
        log(`${fmtPointRef(p)}${p.code ? ` code: ${p.code}` : ""}  layer: ${cad.model.layers.find((l) => l.id === p.layerId)?.name ?? p.layerId}`);
      }
    } else if (sel.type === "linework" && sel.id) {
      ...
      if (t) {
        log(`Text: "${t.text}" at Y ${fmtCoord(t.e)} X ${fmtCoord(t.n)}  layer: ${cad.model.layers.find((l) => l.id === t.layerId)?.name ?? t.layerId}`);
      }
    } else {
  ...
  if (fwd) {
    ...
    log(
      `Forward point ${p.pointNo}: Y ${fmtCoord(res.e)} X ${fmtCoord(res.n)} ` +
  ...
  if (coordMatch) {
    ...
    log(`Point ${created.pointNo}: Y ${fmtCoord(e)} X ${fmtCoord(n)}`);
    return;
  }
