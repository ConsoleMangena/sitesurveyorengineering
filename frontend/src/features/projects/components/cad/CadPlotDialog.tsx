...
export function CadPlotDialog({
  ...
  return (
    <div className="cad-plot-overlay" role="dialog" aria-label="Plot / layout" aria-modal="true">
      <div className="cad-plot-dialog">
        <header className="cad-plot-header">
          <span>{layoutName ? `${layoutName} — paper space` : "Plot layout — printed format"}</span>
          <button type="button" className="cad-panel-collapse" onClick={onClose} title="Close" aria-label="Close">
            <X size={16} />
          </button>
        </header>

        <div className="cad-plot-body">
          {/* ── Controls ─────────────────────────────────────────────── */}
          <div className="cad-plot-controls">
            <div className="cad-settings-group">
              <div className="cad-settings-group-title">Sheet</div>

              <label className="cad-edit-row">
                <span>Paper size</span>
                <select className="input-field" value={opts.paper}
              ...
              <label className="cad-edit-row">
                ...
                <select className="input-field" value={opts.orientation}
              ...
              <label className="cad-edit-row">
                <span>Margin (mm)</span>
                <input className="input-field" type="number" min={4} max={30} value={opts.marginMm}
                  onChange={(e) => set("marginMm", Math.max(4, Math.min(30, Number(e.target.value) || 10)))} />
              </label>
            </div>

            <div className="cad-settings-group">
              <div className="cad-settings-group-title">Viewport (pan / zoom)</div>
              <div className="cad-plot-viewport-pad" role="group" aria-label="Pan and zoom the layout viewport">
                <button type="button" className="cad-vp-btn cad-vp-up" title="Pan up" aria-label="Pan up"
                  onClick={() => pan(0, 1)}><ArrowUp size={13} /></button>
                <button type="button" className="cad-vp-btn cad-vp-left" title="Pan left" aria-label="Pan left"
                  onClick={() => pan(-1, 0)}><ArrowLeft size={13} /></button>
                <button type="button" className="cad-vp-btn cad-vp-center" title="Reset view (Zoom extents)"
                  aria-label="Reset view" onClick={handleFitExtents}><Maximize2 size={12} /></button>
                <button type="button" className="cad-vp-btn cad-vp-right" title="Pan right" aria-label="Pan right"
                  onClick={() => pan(1, 0)}><ArrowRight size={13} /></button>
                <button type="button" className="cad-vp-btn cad-vp-down" title="Pan down" aria-label="Pan down"
                  onClick={() => pan(0, -1)}><ArrowDown size={13} /></button>
              </div>
              <div className="cad-plot-zoom-row">
                <button type="button" className="cad-chip-btn" title="Zoom out" onClick={() => zoomBy(1 / 1.25)}>
                  <ZoomOut size={13} /> Out
                </button>
                <span className="cad-plot-zoom-readout" title="Viewport zoom factor">
                  {(view.zoom * 100).toFixed(0)}%
                </span>
                <button type="button" className="cad-chip-btn" title="Zoom in" onClick={() => zoomBy(1.25)}>
                  <ZoomIn size={13} /> In
                </button>
              </div>
              {viewModified && (
                <button type="button" className="cad-plot-reset-view" onClick={handleFitExtents}>
            ...
            <div className="cad-settings-group">
              <div className="cad-settings-group-title">Sheet elements</div>
            ...
            <div className="cad-settings-group">
              <div className="cad-settings-group-title">Title block</div>
          ...
          <div
            ref={previewWrapRef}
            className={`cad-plot-preview ${dragging ? "dragging" : ""}`}
          ...
          >
            <div
              className="cad-plot-preview-sheet"
              dangerouslySetInnerHTML={{ __html: result.svg }}
            />
            <div className="cad-plot-preview-meta">
              {opts.paper} · {opts.orientation} · 1:{result.denominator}
            </div>
            <div className="cad-plot-preview-hint" aria-hidden="true">
              <Hand size={12} /> Drag to pan · Wheel to zoom
            </div>
          </div>
        </div>

        <footer className="cad-plot-footer">
          <span className="cad-panel-hint" style={{ margin: 0 }}>
            {result.paperW} × {result.paperH} mm · scale 1:{result.denominator} · drag the sheet to pan, wheel to zoom.
          </span>
          <div className="cad-plot-footer-actions">
            <button type="button" className="cad-chip-btn" onClick={handleExportSvg}>
              <Download size={13} /> Export SVG
            </button>
            <button type="button" className="cad-plot-print-btn" onClick={handlePrint}>
