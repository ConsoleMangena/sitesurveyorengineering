...
import { stakeHorizontalCurve, verticalCurve } from "./survey/alignmentBridge.ts";
import {
  lineLine,
  lineArc,
  arcArc,
...
const MODE_LABELS: Record<CogoMode, string> = {
  ...
  "alignment-v": "Vertical Curve (set-out)",
...
const MODE_GROUPS: { label: string; modes: CogoMode[] }[] = [
  ...
  {
    label: "Traverse",
    modes: ["traverse", "angular-traverse", "levelling"],
  },
  {
    label: "Alignment",
    modes: ["alignment-h", "alignment-v"],
  },
  {
    label: "Transform",
    modes: ["scale", "projection", "transform-helmert", "transform-affine"],
  },
  {
    label: "Geometry",
...
function pointRef(model: CadModelState, p: SurveyPoint | null): string {
  return p ? fmtPointRef(p, 3) : "—";
}

export function CadCogoPanel({ cad, model, selection, bearingFormat, angleEntry = "packed", log }: CadCogoPanelProps) {
  ...
  return (
    <div className="cad-panel-block cad-cogo-panel">
      ...
      {mode === "forward" && <ForwardForm {...{ model, bearingFormat, angleEntry, cad, log }} />}
      {mode === "inverse" && <InverseForm {...{ model, bearingFormat, log }} />}
      {mode === "stakeout" && <StakeOutForm {...{ model, bearingFormat, log }} />}
      {mode === "intersection-bb" && <IntersectionBbForm {...{ model, bearingFormat, angleEntry, cad, log }} />}
      {mode === "intersection-dd" && <IntersectionDdForm {...{ model, cad, log }} />}
      {mode === "resection" && <ResectionForm {...{ model, cad, log }} />}
      {mode === "traverse" && <TraverseForm {...{ model, cad, log }} />}
      {mode === "angular-traverse" && <AngularTraverseForm {...{ model, cad, bearingFormat, log }} />}
      {mode === "levelling" && <LevellingForm {...{ log }} />}
      {mode === "alignment-h" && <HorizontalCurveForm {...{ model, cad, bearingFormat, log }} />}
      {mode === "alignment-v" && <VerticalCurveForm {...{ log }} />}
      {mode === "scale" && <ScaleForm />}
      {mode === "projection" && <ProjectionForm {...{ cad, log }} />}
      {mode === "area" && <AreaForm {...{ model, selection, log, cad }} />}
      {mode === "line-line" && <LineLineForm {...{ model, cad, log }} />}
      {mode === "line-arc" && <LineArcForm {...{ model, cad, log }} />}
      {mode === "arc-arc" && <ArcArcForm {...{ model, cad, log }} />}
      {mode === "circle-fit" && <CircleFitForm {...{ model, cad, log }} />}
      {mode === "free-station" && <FreeStationForm {...{ model, cad, bearingFormat, angleEntry, log }} />}
      {mode === "transform-helmert" && <TransformForm kind="helmert" {...{ model, cad, log }} />}
      {mode === "transform-affine" && <TransformForm kind="affine" {...{ model, cad, log }} />}
    </div>
  );
}
...
function ForwardForm({
  ...
  cad,
  log,
}: Pick<CadCogoPanelProps, "model" | "bearingFormat" | "angleEntry" | "cad" | "log">) {
  ...
  const compute = () => {
    ...
    log(
      `Forward point ${p.pointNo}: Y ${fmtCoord(res.e, 3)} X ${fmtCoord(res.n, 3)} ` +
  ...
  return (
    <fieldset className="cad-cogo-group">
      ...
      <PointNumber label="From point" value={fromPno} onChange={setFromPno} model={model} />
...
function InverseForm({
  ...
  bearingFormat,
  log,
}: Pick<CadCogoPanelProps, "model" | "bearingFormat" | "log">) {
  const [fromPno, setFromPno] = useState("");
  ...
  return (
    <fieldset className="cad-cogo-group">
      ...
      <PointNumber label="From point" value={fromPno} onChange={setFromPno} model={model} />
      <PointNumber label="To point" value={toPno} onChange={setToPno} model={model} />
      ...
      {result && (
        <div className="cad-prop-list">
          ...
          <div><span>dX (Northing)</span><strong>{fmtCoord(result.dX, 3)}</strong></div>
          <div><span>dY (Easting)</span><strong>{fmtCoord(result.dY, 3)}</strong></div>
        </div>
      )}
    </fieldset>
  );
...
function StakeOutForm({
  ...
  return (
    <fieldset className="cad-cogo-group">
      ...
      <PointNumber label="Occupied station" value={occ} onChange={setOcc} model={model} />
      <PointNumber label="Backsight station" value={bs} onChange={setBs} model={model} />
      <PointNumber label="Design / target point" value={target} onChange={setTarget} model={model} />
...
function IntersectionBbForm({
  ...
  const compute = () => {
    ...
    log(
      `Intersection BB point ${pt.pointNo}: Y ${fmtCoord(res.e, 3)} X ${fmtCoord(res.n, 3)} ` +
  ...
  return (
    <fieldset className="cad-cogo-group">
      ...
      <PointNumber label="Point 1" value={p1} onChange={setP1} model={model} />
      <TextRow label="Bearing 1" value={brg1} onChange={setBrg1} />
      <PointNumber label="Point 2" value={p2} onChange={setP2} model={model} />
...
function IntersectionDdForm({
  ...
  cad,
  log,
}: Pick<CadCogoPanelProps, "model" | "cad" | "log">) {
  const [p1, setP1] = useState("");
  ...
  const compute = () => {
    ...
    for (const s of sols) {
      ...
      log(`Intersection DD point ${pt.pointNo}: Y ${fmtCoord(s.e, 3)} X ${fmtCoord(s.n, 3)}`);
    }
  };
...
function ResectionForm({
  ...
  const compute = () => {
    ...
    log(`Resection point ${pt.pointNo}: Y ${fmtCoord(res.e, 3)} X ${fmtCoord(res.n, 3)}`);
  };

  return (
    <fieldset className="cad-cogo-group">
      <legend>Three-point resection (Tienstra)</legend>
      <PointNumber label="Station A" value={a} onChange={setA} model={model} />
      <PointNumber label="Station B" value={b} onChange={setB} model={model} />
      <PointNumber label="Station C" value={c} onChange={setC} model={model} />
...
function TraverseForm({
  ...
  return (
    <fieldset className="cad-cogo-group">
      ...
      <PointNumber label="Start point" value={startPno} onChange={setStartPno} model={model} />
      ...
      {type === "closed-link" && (
        <PointNumber label="Closing point" value={closePno} onChange={setClosePno} model={model} />
      )}
          ...
          <tbody>
            {legs.map((leg, i) => (
              <tr key={i}>
                ...
                <td>
                  <button type="button" className="cad-layer-lock" onClick={() => removeLeg(i)} title="Remove leg">×</button>
                </td>
              </tr>
            ))}
          </tbody>
...
function AngularTraverseForm({
  ...
  bearingFormat,
  log,
}: Pick<CadCogoPanelProps, "model" | "cad" | "bearingFormat" | "log">) {
  const [startPno, setStartPno] = useState("");
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i}>
                ...
                <td><button type="button" className="cad-layer-lock" onClick={() => removeRow(i)} title="Remove">×</button></td>
              </tr>
            ))}
          </tbody>
        </table>
...
function HorizontalCurveForm({
  ...
  return (
    <fieldset className="cad-cogo-group">
      ...
      <PointNumber label="PI (intersection point)" value={piPno} onChange={setPiPno} model={model} />
...
// ─────────────────────────────────────────────────────────────────────────────
// Vertical parabolic curve set-out
// ─────────────────────────────────────────────────────────────────────────────
function VerticalCurveForm({ log }: { log: CadCogoPanelProps["log"] }) {
  const [bvc, setBvc] = useState("");
  const [g1, setG1] = useState("");
  const [g2, setG2] = useState("");
  const [length, setLength] = useState("");
  const [interval, setInterval] = useState("20");
  const [result, setResult] = useState<Awaited<ReturnType<typeof verticalCurve>>>(null);

  const compute = async () => {
    const b = parseFloat(bvc);
    const gg1 = parseFloat(g1);
    const gg2 = parseFloat(g2);
    const len = parseFloat(length);
    const iv = parseFloat(interval);
    if ([b, gg1, gg2, len].some((x) => !Number.isFinite(x))) { log("Vertical curve: all fields must be numeric.", "error"); return; }
    if (len <= 0) { log("Vertical curve: length must be positive.", "error"); return; }
    const res = await verticalCurve(b, gg1, gg2, len, Number.isFinite(iv) && iv > 0 ? iv : 0);
    if (!res) { log("Vertical curve: invalid parameters.", "error"); return; }
    setResult(res);
    log(
      `Vertical curve g1 ${gg1}% → g2 ${gg2}%, L ${len} m — ` +
        `BVC ${res.bvcElevation.toFixed(3)} → EVC ${res.evcElevation.toFixed(3)} m` +
        (res.turningChainage != null ? `, turning @ ${res.turningChainage.toFixed(2)} m (${res.turningElevation?.toFixed(3)} m)` : ""),
    );
  };

  return (
    <fieldset className="cad-cogo-group">
      <legend>Vertical parabolic curve</legend>
      <NumberRow label="BVC elevation (m)" value={bvc} onChange={setBvc} />
      <NumberRow label="Grade in g₁ (%)" value={g1} onChange={setG1} />
      <NumberRow label="Grade out g₂ (%)" value={g2} onChange={setG2} />
      <NumberRow label="Curve length L (m)" value={length} onChange={setLength} />
      <NumberRow label="Chainage interval (m)" value={interval} onChange={setInterval} />
      <button className="cad-chip-btn" type="button" onClick={() => void compute()}>Design curve</button>
      {result && (
        <>
          <div className="cad-prop-list" style={{ marginTop: 8 }}>
            <div><span>BVC / EVC</span><strong>{result.bvcElevation.toFixed(3)} / {result.evcElevation.toFixed(3)} m</strong></div>
            <div><span>Grade change A</span><strong>{result.gradeChange.toFixed(3)} %</strong></div>
            <div><span>Turning point</span><strong>{result.turningChainage != null ? `${result.turningChainage.toFixed(2)} m @ ${result.turningElevation?.toFixed(3)} m` : "outside curve"}</strong></div>
          </div>
          {result.stations.length > 0 && (
            <div className="cad-cogo-table-wrap" style={{ marginTop: 8 }}>
              <table className="cad-cogo-table">
                <thead><tr><th>Chainage (m)</th><th>RL (m)</th></tr></thead>
                <tbody>
                  {result.stations.map((s, i) => (
                    <tr key={i}><td>{s.chainage.toFixed(2)}</td><td>{s.elevation.toFixed(3)}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
...
// Levelling
// ─────────────────────────────────────────────────────────────────────────────
function LevellingForm({ log }: { log: CadCogoPanelProps["log"] }) {
  ...
  const [rows, setRows] = useState<{ label: string; kind: StaffKind; reading: string }[]>([
    { label: "BM", kind: "BS", reading: "" },
  ]);
  const [result, setResult] = useState<ReturnType<typeof reduceLevelling> | null>(null);

  const addRow = (kind: StaffKind = "IS") => setRows((prev) => [...prev, { label: "", kind, reading: "" }]);
  const updateRow = (idx: number, patch: Partial<(typeof rows)[number]>) =>
    setRows((prev) => prev.map((r, i) => (i === idx ? { ...r, ...patch } : r)));
  const removeRow = (idx: number) => setRows((prev) => prev.filter((_, i) => i !== idx));

  const compute = () => {
    const bm = parseFloat(startRL);
    if (!Number.isFinite(bm)) { log("Levelling: start RL must be numeric.", "error"); return; }
    const readings: LevellingReading[] = rows.map((r, i) => ({
      label: r.label || `Stn ${i + 1}`,
      kind: r.kind,
      reading: parseFloat(r.reading),
    }));
    if (readings.some((r) => !Number.isFinite(r.reading))) { log("Levelling: all readings must be numeric.", "error"); return; }
    const knownClose = closingRL.trim() === "" ? undefined : parseFloat(closingRL);
    const res = reduceLevelling(readings, bm, method, knownClose);
    setResult(res);
    log(
      `Levelling (${res.method}) — ΣBS ${res.sumBS.toFixed(3)} m, ΣFS ${res.sumFS.toFixed(3)} m, ` +
        `last RL ${res.rows[res.rows.length - 1]?.rl.toFixed(3) ?? "—"} m${
          res.misclose != null ? `, misclosure ${res.misclose.toFixed(3)} m` : ""
        }`,
    );
  };
  ...
  return (
    <fieldset className="cad-cogo-group">
      <legend>Levelling reduction</legend>
      <div className="cad-cogo-field">
        <span>Method</span>
        <select className="input-field" value={method} onChange={(e) => setMethod(e.target.value as typeof method)}>
          <option value="rise-fall">Rise &amp; Fall</option>
          <option value="hpc">Height of Plane of Collimation</option>
        </select>
      </div>
      <NumberRow label="Start RL (m)" value={startRL} onChange={setStartRL} />
      <NumberRow label="Known closing RL (m) — optional" value={closingRL} onChange={setClosingRL} />

      <div className="cad-cogo-table-wrap">
        <table className="cad-cogo-table">
          <thead>
            <tr><th>Label</th><th>Kind</th><th>Reading</th><th /></tr>
          </thead>
          <tbody>
            {rows.map((r, i) => (
              <tr key={i}>
                <td><input className="input-field" value={r.label} onChange={(e) => updateRow(i, { label: e.target.value })} /></td>
                <td>
                  <select className="input-field" value={r.kind} onChange={(e) => updateRow(i, { kind: e.target.value as StaffKind })}>
                    <option value="BS">BS</option>
                    <option value="IS">IS</option>
                    <option value="FS">FS</option>
                  </select>
                </td>
                <td><input className="input-field" type="number" value={r.reading} onChange={(e) => updateRow(i, { reading: e.target.value })} /></td>
                <td><button type="button" className="cad-layer-lock" onClick={() => removeRow(i)}>×</button></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
        <button type="button" className="cad-chip-btn" onClick={() => addRow("IS")}>Add IS</button>
        <button type="button" className="cad-chip-btn" onClick={() => addRow("FS")}>Add FS/TP</button>
        <button type="button" className="cad-chip-btn" onClick={compute}>Reduce</button>
      </div>
      ...
      {result && (
        <div className="cad-cogo-table-wrap" style={{ marginTop: 8 }}>
          <table className="cad-cogo-table">
            <thead>
              <tr><th>Pt</th><th>BS</th><th>IS</th><th>FS</th><th>RL</th><th>Adj RL</th></tr>
            </thead>
            <tbody>
              {result.rows.map((r, i) => (
                <tr key={i}>
                  <td>{r.label}</td>
                  <td>{r.bs?.toFixed(3) ?? "—"}</td>
                  <td>{r.is?.toFixed(3) ?? "—"}</td>
                  <td>{r.fs?.toFixed(3) ?? "—"}</td>
                  <td>{r.rl.toFixed(3)}</td>
                  <td>{r.adjustedRl.toFixed(3)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          <p className="cad-panel-hint" style={{ marginTop: 4 }}>
            Arithmetic check: {result.checkOk ? "OK" : "FAIL"} · ΣBS−ΣFS: {result.bsMinusFs.toFixed(3)} m
          </p>
        </div>
      )}
    </fieldset>
...
// ─────────────────────────────────────────────────────────────────────────────
function ScaleForm() {
  const [pointSF, setPointSF] = useState("1");
  const [height, setHeight] = useState("0");
  const [ground, setGround] = useState("");
  const [grid, setGrid] = useState("");

  const computeFromGround = () => {
    const k = parseFloat(pointSF);
    const h = parseFloat(height);
    const g = parseFloat(ground);
    if ([k, h, g].some((x) => !Number.isFinite(x))) return;
    const csf = combinedScaleFactor(k, h);
    setGrid(groundToGrid(g, csf).toFixed(4));
  };

  const computeFromGrid = () => {
    const k = parseFloat(pointSF);
    const h = parseFloat(height);
    const gv = parseFloat(grid);
    if ([k, h, gv].some((x) => !Number.isFinite(x))) return;
    const csf = combinedScaleFactor(k, h);
    setGround(gridToGround(gv, csf).toFixed(4));
  };
  ...
  const csf = useMemo(() => {
    ...
    return combinedScaleFactor(k, h);
  }, [pointSF, height]);

  return (
    <fieldset className="cad-cogo-group">
      <legend>Combined scale factor</legend>
      <NumberRow label="Point (grid) scale factor" value={pointSF} onChange={setPointSF} />
      <NumberRow label="Height above ellipsoid/MSL (m)" value={height} onChange={setHeight} />
      <div className="cad-prop-list" style={{ marginBottom: 6 }}>
        <div><span>Combined SF</span><strong>{csf != null ? csf.toFixed(8) : "—"}</strong></div>
      </div>
      <div className="cad-cogo-field" style={{ flexDirection: "row", gap: 6, alignItems: "center" }}>
        <input className="input-field" type="number" value={ground} onChange={(e) => setGround(e.target.value)} placeholder="Ground distance" />
        <button type="button" className="cad-chip-btn" onClick={computeFromGround}>→ Grid</button>
      </div>
      <div className="cad-cogo-field" style={{ flexDirection: "row", gap: 6, alignItems: "center" }}>
        <input className="input-field" type="number" value={grid} onChange={(e) => setGrid(e.target.value)} placeholder="Grid distance" />
        <button type="button" className="cad-chip-btn" onClick={computeFromGrid}>→ Ground</button>
      </div>
    </fieldset>
  );
}
...
function ProjectionForm({
  cad,
  log,
}: Pick<CadCogoPanelProps, "cad" | "log">) {
  ...
  const computeForward = () => {
    ...
    log(
      `Projected point ${pt.pointNo} (${proj.label}): Y ${fmtCoord(r.e, 3)} X ${fmtCoord(r.n, 3)} · ` +
  ...
  const computeInverse = () => {
    ...
    if (!Number.isFinite(e) || !Number.isFinite(n)) {
      log("Projection: grid Y/X must be numeric.", "error");
      return;
    }
    ...
    <fieldset className="cad-cogo-group">
      ...
      {fwdResult && (
        <div className="cad-prop-list" style={{ marginTop: 8 }}>
          ...
          <div><span>Y (Easting)</span><strong>{fmtCoord(fwdResult.e, 3)}</strong></div>
          <div><span>X (Northing)</span><strong>{fmtCoord(fwdResult.n, 3)}</strong></div>
      ...
      <NumberRow label="Grid Y / Easting (m)" value={gridE} onChange={setGridE} />
      <NumberRow label="Grid X / Northing (m)" value={gridN} onChange={setGridN} />
...
function LineLineForm({ model, cad, log }: Pick<CadCogoPanelProps, "model" | "cad" | "log">) {
  const [p1, setP1] = useState("");
  const [p2, setP2] = useState("");
  ...
  const compute = async () => {
    ...
    log(`Line–Line intersection ${pt.pointNo}: Y ${fmtCoord(res.e, 3)} X ${fmtCoord(res.n, 3)}`);
  };

  return (
    <fieldset className="cad-cogo-group">
      <legend>Line–line intersection</legend>
      <PointNumber label="Line 1 start" value={p1} onChange={setP1} model={model} />
      <PointNumber label="Line 1 end" value={p2} onChange={setP2} model={model} />
      <PointNumber label="Line 2 start" value={p3} onChange={setP3} model={model} />
      <PointNumber label="Line 2 end" value={p4} onChange={setP4} model={model} />
      <button type="button" className="cad-chip-btn primary" onClick={() => void compute()}>Compute &amp; place point</button>
    </fieldset>
  );
}

function LineArcForm({ model, cad, log }: Pick<CadCogoPanelProps, "model" | "cad" | "log">) {
  const [a, setA] = useState("");
  const [b, setB] = useState("");
  ...
  const compute = async () => {
    ...
    for (const s of sols) {
      ...
      log(`Line–Arc intersection ${pt.pointNo}: Y ${fmtCoord(s.e, 3)} X ${fmtCoord(s.n, 3)}`);
    }
  };
  ...
  return (
    <fieldset className="cad-cogo-group">
      ...
      <PointNumber label="Point on line" value={a} onChange={setA} model={model} />
      <PointNumber label="Second point on line" value={b} onChange={setB} model={model} />
      <PointNumber label="Circle centre" value={centrePno} onChange={setCentrePno} model={model} />
      <NumberRow label="Radius (m)" value={radius} onChange={setRadius} />
...
function ArcArcForm({ model, cad, log }: Pick<CadCogoPanelProps, "model" | "cad" | "log">) {
  ...
  const compute = async () => {
    ...
    for (const s of sols) {
      ...
      log(`Arc–Arc intersection ${pt.pointNo}: Y ${fmtCoord(s.e, 3)} X ${fmtCoord(s.n, 3)}`);
    }
  };
  ...
  return (
    <fieldset className="cad-cogo-group">
      ...
      <PointNumber label="Centre 1" value={c1} onChange={setC1} model={model} />
      <NumberRow label="Radius 1 (m)" value={r1} onChange={setR1} />
      <PointNumber label="Centre 2" value={c2} onChange={setC2} model={model} />
...
function FreeStationForm({
  ...
  angleEntry = "packed",
  log,
}: Pick<CadCogoPanelProps, "model" | "cad" | "bearingFormat" | "angleEntry" | "log">) {
  ...
  const compute = async () => {
    ...
    log(
      `Free-station ${pt.pointNo}: Y ${fmtCoord(res.position.e, 3)} X ${fmtCoord(res.position.n, 3)} — ` +
  ...
  return (
    <fieldset className="cad-cogo-group">
      ...
      <datalist id="cogo-pts-free-stn">
        {model.points.map((p) => (
          <option key={p.id} value={p.pointNo}>{p.pointNo} N{p.n.toFixed(2)} E{p.e.toFixed(2)}</option>
        ))}
      </datalist>
      ...
      {result && (
        <div className="cad-prop-list" style={{ marginTop: 8 }}>
          <div><span>Position Y</span><strong>{fmtCoord(result.position.e, 3)}</strong></div>
          <div><span>Position X</span><strong>{fmtCoord(result.position.n, 3)}</strong></div>
...
function TransformForm({
  ...
  const compute = async () => {
    ...
    if (!source || !target || source.length !== target.length || source.length < (kind === "helmert" ? 2 : 3)) {
      log(`Transform: need ${kind === "helmert" ? "2+" : "3+"} matching (X,Y) pairs.`, "error");
      return;
    }
    if (kind === "helmert") {
  ...
  const apply = async () => {
    ...
    log(`Transformed ${p.pointNo} → ${pt.pointNo}: Y ${fmtCoord(res.e, 3)} X ${fmtCoord(res.n, 3)}`);
  };
  ...
  return (
    <fieldset className="cad-cogo-group">
      ...
      <TextRow
        label="Source Y,X list"
        value={sourceList}
        onChange={setSourceList}
        placeholder="N1,E1,N2,E2,N3,E3"
      />
      <TextRow
        label="Target Y,X list"
        value={targetList}
      ...
      {transform && diagnostics && (
        <>
          ...
          <div className="cad-cogo-field" style={{ flexDirection: "row", gap: 6, alignItems: "center", marginTop: 10 }}>
            <PointNumber label="Apply to point" value={applyPno} onChange={setApplyPno} model={model} />
...
function PointNumber({
  ...
  return (
    <label className="cad-cogo-field">
      ...
      <span style={{ fontSize: 10, color: "var(--cad-text-dim)" }}>{pointRef(model, resolved)}</span>
    </label>
  );
}
