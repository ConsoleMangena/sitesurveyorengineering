...
export function CadViewport({
  ...
  const handleMouseMove = (ev: React.MouseEvent) => {
    ...
    if (pendingVertices.length > 0 && (tool === "line" || tool === "polyline" || tool === "boundary" || tool === "measure")) {
      const last = pendingVertices[pendingVertices.length - 1];
        ...
        for (let i = 1; i < lw.vertices.length; i++) {
          ...
          labels.push(
            <text key={`${lw.id}-lbl-${i}`} x={mid.x + offX} y={mid.y + offY - 3}
              fill="#90a5c4" fontSize={9} textAnchor="middle" className="cad-seg-label">
      ...
      return (
        <g key={lw.id}>
          {selected && (
            <path
              d={d}
              fill={lw.closed ? "rgba(92, 195, 255, 0.06)" : "none"}
              stroke="#5cc3ff"
              strokeWidth={sw + 1}
              strokeDasharray="3 3"
              opacity={0.7}
    ...
    return model.points.map((p) => {
      ...
      return (
        <g key={p.id}>
          ...
          {showPointLabels && (
            <text x={s.x + 7} y={s.y - 5} fill="#a0b0c8" fontSize={9}
  ...
  return (
    ...
    >
      ...
      <div className="cad-ucs-icon" aria-hidden="true">
        <svg width="44" height="44" viewBox="0 0 44 44">
          ...
          <text x="35" y="38" fill="#e06c75" textAnchor="start" fontSize="11"
            fontFamily="'Segoe UI', system-ui, sans-serif" fontWeight="700">{axisLabels(axisConvention).easting}</text>
          <text x="4" y="7" fill="#3bb46e" textAnchor="middle" fontSize="11"
            fontFamily="'Segoe UI', system-ui, sans-serif" fontWeight="700">{axisLabels(axisConvention).northing}</text>
        </svg>
      </div>
