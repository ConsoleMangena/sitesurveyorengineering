...
export function Cad3dViewport({
  ...
  const bounds = useMemo<Bounds | null>(() => {
    ...
    for (const s of model.surfaces) for (const v of s.points) acc(v.e, v.n, v.z);
  ...
  useEffect(() => {
    ...
    for (const srf of model.surfaces) {
      ...
      if (cf) for (const ct of cf.triangles) cfByKey.set(`${ct.a},${ct.b},${ct.c}`, ct.delta);
      const ss = srf.slopeShade;
      const ssByKey = new Map<string, string>();
      if (ss) for (const st of ss.triangles) ssByKey.set(`${st.a},${st.b},${st.c}`, st.color);
