...
public jobs/firms/training views + seeds). `0001_schema.sql` ends with
`13_chat.sql` (team chat) and `14_ai_chat.sql` (per-account SiteSurveyor AI
agent conversations). `9999_reset_and_apply_all.sql` is a dev-only one-shot
